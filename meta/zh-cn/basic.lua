...
---
---接收任意数量的参数，并将它们的值以 `[neverlose]` 格式将消息打印到 `console` 。 它用 `tostring` 函数将每个参数都转换为字符串。
---
function print(...) end
...
---
---接收任意数量的参数，并将它们的值以 `[neverlose]` 格式将消息打印到 `console` 并播放错误提示音。 它用 `tostring` 函数将每个参数都转换为字符串。
---
function print_error(...) end

---
---接收任意数量的参数，并将它们的值打印到游戏中本地客户端的 `chat` 。 它用 `tostring` 函数将每个参数都转换为字符串。
---
function print_chat(...) end

---接收任意数量的参数，并将它们的值打印到 `console` 。 它用 `tostring` 函数将每个参数都转换为字符串。可使用 `\a` 转义字符后跟着16进制，更改颜色类似于
---```lua
---print_raw("\aFF0000RR", "\a00FF00GG", "\a0000FFBB")
---```
---
function print_raw(...) end

---接收任意数量的参数，并将它们的值打印到 `upper-left console` 。 它用 `tostring` 函数将每个参数都转换为字符串。可使用 `\a` 转义字符后跟着16进制，更改颜色，以 `\0` 结尾字符串以防止它添加换行符类似于<br>
---```lua
---print_dev("\a4B69FF[neverlose]\aFF4040\x20\0")
---print_dev("Hello World")
---```
---
function print_dev(...) end
...
---
---能够在发生错误时启用调试回溯和自动脚本卸载。 在脚本开头将全局变量 _DEBUG 设置为 true。
---
---@type boolean
_DEBUG = false

---
---一个包含有当前脚本ID的全局变量（并非函数）。
---
---@type number
_ID = 0

---
---显示脚本是否发布在了 [`neverlose marketplace`](https://en.neverlose.cc/market/?type=2)（并非函数）。
---
---@type boolean
_IS_MARKET = false

---
---一个包含有当前脚本名字的全局变量（并非函数）。
---
---@type string
_NAME = "Script"
